...
def load_mnist(normalize=True, download_dir="data"):
    """
    ...
    normalize : bool, optional
        Whether to normalize the data to the range [0, 1] (default is True).
    ...
    if not os.path.isfile(local_file):
        print(f"Downloading MNIST dataset to {local_file}...")
        with requests.get(
            "https://s3.amazonaws.com/img-datasets/mnist.npz", stream=True, verify=True
        ) as r:
            with open(local_file, "wb") as f:
                shutil.copyfileobj(r.raw, f)
        print("Download complete.")
    else:
        print(f"MNIST dataset already found at {local_file}.")
...
def generate_data(
    ...
    if dataset_name.lower() == "mnist":
        data = load_mnist(download_dir=str(raw_data_parent_dir))
    else:
        raise ValueError(
            f"Unsupported dataset: {dataset_name}. Only 'mnist' is supported by this script."
        )
    ...
    if part == "iid":
        ...
        try:
            ...
            if not (1 <= labels_per_client <= 10):  # MNIST has 10 labels
                raise ValueError(
                    "Labels per client for non-IID MNIST must be between 1 and 10."
                )
        except ValueError:
            raise ValueError(
    ...
    for i, node_data in party_datas.items():
        ...
        file_path = node_specific_folder / f"{dataset_name.lower()}.npz"
...
def main(
    ...
    raw_data_folder: Path = Path(__file__).parent.parent / "raw",
    ...
    generate_data(
        ...
        part=partition_type,
        raw_data_parent_dir=raw_data_folder,  # Pass base so generate_data can construct .../raw_data/mnist
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download and partition MNIST dataset for federated learning."
    )
    parser.add_argument(
        "-n",
        "--n_workers",
    ...
    parser.add_argument(
        ...
        "--raw_data_folder",
        type=str,
        default="temp/raw",  # Default output folder within the example directory
